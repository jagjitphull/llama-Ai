...
with gr.Blocks(title="🦙 Smart AI Knowledge Center — Llama 3, CodeLlama, and Google Automation by GNU IT Solutions & Services") as demo:
    ...
    .gr-button, button, input[type="button"], .gr-text-input, .gr-dropdown, .gr-file, .gr-radio, .gr-number, .gr-markdown {
        ...
        color: #fff !important;
        border: 1px solid #363b40 !important;
    }
    .gr-button, button {
        background: linear-gradient(90deg, #323a44 0%, #1f584e 100%) !important;
    }
    .gr-button:hover, button:hover {
        background: #1f584e !important;
    }
    .gr-text-input, .gr-dropdown, .gr-radio, .gr-number, .gr-file {
        border-radius: 7px !important;
    }
    a, a:visited { color: #4bd9c2 !important; }
    ::-webkit-scrollbar { width: 10px; background: #23272c; }
    ::-webkit-scrollbar-thumb { background: #3a4250; border-radius: 8px; }
    </style>
    """)

    # LOGO AND BANNER
    gr.Markdown(f"""
    <div style="background: linear-gradient(90deg, #23272c 0%, #1f584e 100%); color: white; padding: 16px 20px 12px 20px; border-radius: 16px; box-shadow: 0 8px 24px rgba(40,80,110,0.18); margin-bottom: 16px; text-align: center; display:flex; align-items:center; flex-direction:column;">
        <img src="file/{LOGO_URL}" alt="GNU IT Solutions & Services Logo" style="height:60px; margin-bottom:10px;">
        <h1 style="font-size:2.3em; margin-bottom:0.2em;">🦙 Smart AI Knowledge Center</h1>
        <h2 style="font-weight:400; font-size:1.15em; color:#4bd9c2;">Llama 3, CodeLlama & Google Actions<br>
    ...
    gr.Markdown(
        '<div style="text-align:center; color:#666; margin-top:24px; font-size:1em;">© 2024 GNU IT Solutions & Services. All rights reserved.</div>'
    )
